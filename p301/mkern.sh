...
#ROOTFS=$1
ROOTFS="out/target/product/p301/ramdisk.img"
...
KERNEL_OUT=out/target/product/p301/obj/KERNEL_OBJ
#mkdir -p $KERNEL_OUT
...
if [ "$2" != "m" ]; then
    out/host/linux-x86/bin/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm64/boot/Image \
        --ramdisk ${ROOTFS} \
        --second common/../$KERNEL_OUT/arch/arm64/boot/dts/amlogic/gxtvbb_p301.dtb \
        --output ./out/target/product/p301/boot.img
    ls -l ./out/target/product/p301/boot.img
    echo "boot.img done"
fi
