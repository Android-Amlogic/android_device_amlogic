...
#{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{
KERNEL_DEFCONFIG=meson6_g39emmc7_jb_defconfig
KERNEL_USB_BURNING_DEFCONFIG=meson6_g39emmc7_jb_usb_burn_defconfig
KERNET_ROOTDIR=common
PROJECT_NAME=g39refemmc7
#}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}
...
NEW_KERNEL_CONFIG=
DEVICE_DIR=device/amlogic/g39refemmc7
...
function usage () {
	...
	echo "   device/amlogic/g39refemmc7/quick_build_kernel.sh bootimage [kernel config]     --> build boot.img"
	echo "   device/amlogic/g39refemmc7/quick_build_kernel.sh recoveryimage [kernel config]  --> recovery.img"
	echo "   device/amlogic/g39refemmc7/quick_build_kernel.sh usbburnimage                  --> uImage_usb_burning"
	echo "   device/amlogic/g39refemmc7/quick_build_kernel.sh menuconfig                    --> open kernel menuconfig"
	echo "   device/amlogic/g39refemmc7/quick_build_kernel.sh saveconfig                    --> make defconfig &  make savedefconfig"
	echo "   device/amlogic/g39refemmc7/quilk_build_kernel.sh usbburnzip                 --> build usbburnzip.zip"
}
...
mkdir $PRODUCT_OUT/g39refemmc7_usb_burn
echo "copy usb burning file......."
echo "copy config_progress.xml....."
cp -f $DEVICE_DIR/config_progress.xml $PRODUCT_OUT/g39refemmc7_usb_burn
echo "copy u-boot-orig.bin....."
cp -f $DEVICE_DIR/u-boot-orig.bin $PRODUCT_OUT/g39refemmc7_usb_burn
echo "copy usb_spl.bin....."
cp -f $DEVICE_DIR/usb_spl.bin $PRODUCT_OUT/g39refemmc7_usb_burn
echo "copy update.zip......"
cp -f $PRODUCT_OUT/g39refemmc7-ota* $PRODUCT_OUT/g39refemmc7_usb_burn/update.zip
echo "copy usb_burning.img...."
mv -f $PRODUCT_OUT/usb_burning.img $PRODUCT_OUT/g39refemmc7_usb_burn
echo "copy u-boot-orig-usid.bin....."
cp -f $DEVICE_DIR/u-boot-orig-usid.bin $PRODUCT_OUT/g39refemmc7_usb_burn

if [ ! -f $PRODUCT_OUT/g39refemmc7_usb_burn/config_progress.xml ]; then
	echo "###ERROR!! No $PRODUCT_OUT/g39refemmc7_usb_burn/config_progress.xml found! check the file in you device first "
exit 1
fi
if [ ! -f $PRODUCT_OUT/g39refemmc7_usb_burn/u-boot-orig-usid.bin ]; then
	echo "###ERROR!! No $PRODUCT_OUT/g39refemmc7_usb_burn/u-boot-orig-usid.bin found! check the file in you device first "
exit 1
fi
if [ ! -f $PRODUCT_OUT/g39refemmc7_usb_burn/u-boot-orig.bin ]; then
	echo "###ERROR!! No $PRODUCT_OUT/g39refemmc7_usb_burn/u-boot-orig.bin found! check the file in you device first"
exit 1
fi
if [ ! -f $PRODUCT_OUT/g39refemmc7_usb_burn/usb_spl.bin ]; then
	echo "###ERROR!! No $PRODUCT_OUT/g39refemmc7_usb_burn/usb_spl.bin found! check the file in you device first"
exit 1
fi
if [ ! -f $PRODUCT_OUT/g39refemmc7_usb_burn/usb_burning.img ]; then
	echo "###ERROR!! No $PRODUCT_OUT/g39refemmc7_usb_burn/usb_burning.img found! maybe Cause by compiled usb_burning.img fail please check config"
exit 1
fi
if [ ! -f $PRODUCT_OUT/g39refemmc7_usb_burn/update.zip ]; then
	echo "###ERROR!! No $PRODUCT_OUT/g39refemmc7_usb_burn/update.zip found! please do "make otapackage -j24 first ""
exit 1
fi
if [ -f $PRODUCT_OUT/$DATE-G39-shuttle-emmc7-usb-burn.zip ]; then
echo "###we will rm the before one ... "
rm -f $PRODUCT_OUT/$DATE-G39-shuttle-emmc7-usb-burn.zip
fi
cd $PRODUCT_OUT

echo "creating zip ......"
zip -r $DATE-G39-shuttle-emmc7-usb-burn.zip g39refemmc7_usb_burn/
rm -rf g39refemmc7_usb_burn/

echo "Build $PRODUCT_OUT/$DATE-G39-shuttle-emmc7-usb-burn.zip Done"
exit 0
fi
