...
#ROOTFS=$1
ROOTFS="out/target/product/gxtvbb_skt/ramdisk.img"
...
KERNEL_OUT=out/target/product/gxtvbb_skt/obj/KERNEL_OBJ
#mkdir -p $KERNEL_OUT
...
if [ "$2" != "m" ]; then
#    make -C common O=../$KERNEL_OUT gxtvbb_skt.dtd ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
    make -C common O=../$KERNEL_OUT gxtvbb_skt.dtb ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
fi
...
if [ "$2" != "m" ]; then
    out/host/linux-x86/bin/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm64/boot/Image \
        --ramdisk ${ROOTFS} \
        --second common/../$KERNEL_OUT/arch/arm64/boot/dts/amlogic/gxtvbb_skt.dtb \
        --output ./out/target/product/gxtvbb_skt/boot.img
    ls -l ./out/target/product/gxtvbb_skt/boot.img
    echo "boot.img done"
fi
