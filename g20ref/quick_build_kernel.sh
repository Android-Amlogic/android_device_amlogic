...
function usage () {
	...
	echo "   device/amlogic/g20ref/quick_build_kernel.sh bootimage [kernel config]     --> build boot.img"
	echo "   device/amlogic/g20ref/quick_build_kernel.sh recoveryimage [kernel config]  --> recovery.img"
	echo "   device/amlogic/g20ref/quick_build_kernel.sh usbburnimage                  --> uImage_usb_burning"
	echo "   device/amlogic/g20ref/quick_build_kernel.sh menuconfig                    --> open kernel menuconfig"
	echo "   device/amlogic/g20ref/quick_build_kernel.sh saveconfig                    --> make defconfig &  make savedefconfig"
}
...
#{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{
KERNEL_DEFCONFIG=meson6_g20_jb42_defconfig
KERNEL_USBBURN_CONFIG=meson6_g20_usb_burning_defconfig
...
cd $PRODUCT_OUT/recovery/root
find .| cpio -o -H newc | gzip -9 > ../../ramdisk-usbburning.img
cd -
out/host/linux-x86/bin/mkbootimg  --kernel $KERNEL_OUT/arch/arm/boot/uImage  --ramdisk $PRODUCT_OUT/ramdisk-usbburning.img --output $PRODUCT_OUT/uImage_usb_burning
