...
function usage () {
	...
	echo "   device/amlogic/g19ref/quick_build_kernel.sh bootimage [kernel config]     --> build boot.img"
	echo "   device/amlogic/g19ref/quick_build_kernel.sh recoveryimage [kernel config] --> recovery.img"
	echo "   device/amlogic/g19ref/quick_build_kernel.sh usbburnimage                  --> uImage_usb_burning"
	echo "   device/amlogic/g19ref/quick_build_kernel.sh menuconfig                    --> open kernel menuconfig"
	echo "   device/amlogic/g19ref/quick_build_kernel.sh saveconfig                    --> make defconfig &  make savedefconfig"
	echo "   device/amlogic/g19ref/quick_build_kernel.sh patch                         --> make patch with bootimg"
}
...
#{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{
KERNEL_DEFCONFIG=meson6_g19_defconfig
KERNEL_USBBURN_CONFIG=meson6_g19_usb_burning_defconfig
...
make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-eabi- uImage -j9
make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-eabi- modules -j9
...
if [ $1 = patch ]; then
    ...
    mkdir -p inputdir/SYSTEM/lib
    cp system/lib/dhd.ko inputdir/SYSTEM/lib/dhd.ko
...
make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-eabi- $KERNEL_USBBURN_CONFIG

make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-eabi- -j9
mkdir -p $PRODUCT_OUT/recovery/root/boot
