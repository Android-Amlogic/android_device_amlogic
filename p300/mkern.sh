...
#ROOTFS=$1
ROOTFS="out/target/product/p300/ramdisk.img"
...
KERNEL_OUT=out/target/product/p300/obj/KERNEL_OBJ
#mkdir -p $KERNEL_OUT
...
if [ "$2" != "m" ]; then
    out/host/linux-x86/bin/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm64/boot/Image \
        --ramdisk ${ROOTFS} \
        --second common/../$KERNEL_OUT/arch/arm64/boot/dts/amlogic/p300.dtb \
        --output ./out/target/product/p300/boot.img
    ls -l ./out/target/product/p300/boot.img
    echo "boot.img done"
fi
