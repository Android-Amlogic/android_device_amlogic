...
function usage () {
	...
	echo "   device/amlogic/g18refmmcspi/quick_build_kernel.sh bootimage [kernel config]     --> build uImage"
	echo "   device/amlogic/g18refmmcspi/quick_build_kernel.sh recoveryimage [kernel config]  --> build recovery uImage"
	echo "   device/amlogic/g18refmmcspi/quick_build_kernel.sh usbburnimage                  --> uImage_usb_burning"
	echo "   device/amlogic/g18refmmcspi/quick_build_kernel.sh menuconfig                    --> open kernel menuconfig"
	echo "   device/amlogic/g18refmmcspi/quick_build_kernel.sh saveconfig                    --> savedefconfig"
}
...
#{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{{
KERNEL_DEFCONFIG=meson6_g18_mmcspi_jbmr1_defconfig
KERNEL_USBBURN_CONFIG=meson6_g18_mmcspi_usb_burning_defconfig
KERNET_ROOTDIR=common
PROJECT_NAME=g18refmmcspi
#}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}
...
if [ -n $NEW_KERNEL_CONFIG ]; then
	make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-none-linux-gnueabi- $NEW_KERNEL_CONFIG -j4
else
	if [ -f $KERNEL_CONFIG ]; then
		make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-none-linux-gnueabi- $KERNEL_DEFCONFIG -j4
	fi		
fi
...
make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-none-linux-gnueabi- uImage -j4
...
make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-none-linux-gnueabi- BUILD_URECOVERY=true uImage -j4
make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-none-linux-gnueabi- modules
...
make -C $KERNET_ROOTDIR O=../$KERNEL_OUT ARCH=arm CROSS_COMPILE=arm-none-linux-gnueabi- $KERNEL_USBBURN_CONFIG
