...
#ROOTFS=$1
ROOTFS="out/target/product/p200_2G/ramdisk.img"
...
KERNEL_OUT=out/target/product/p200_2G/obj/KERNEL_OBJ
#mkdir -p $KERNEL_OUT
...
if [ "$2" != "m" ]; then
#    make -C common O=../$KERNEL_OUT gxbb_p200_2G.dtd ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
    make -C common O=../$KERNEL_OUT gxbb_p200_2G.dtb ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
fi
...
if [ "$2" != "m" ]; then
    out/host/linux-x86/bin/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm64/boot/Image \
        --ramdisk ${ROOTFS} \
        --second common/../$KERNEL_OUT/arch/arm64/boot/dts/amlogic/gxbb_p200_2G.dtb \
        --output ./out/target/product/p200_2G/boot.img
    ls -l ./out/target/product/p200_2G/boot.img
    echo "boot.img done"
fi
