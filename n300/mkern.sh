...
KERNEL_OUT=out/target/product/n300/obj/KERNEL_OBJ
mkdir -p $KERNEL_OUT
...
if [ "$2" != "m" ]; then
    make -C common O=../$KERNEL_OUT mesong9tv_n300.dtd
    make -C common O=../$KERNEL_OUT mesong9tv_n300.dtb
fi
...
if [ "$2" != "m" ]; then
    common/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm/boot/uImage \
        --ramdisk ${ROOTFS} \
        --second common/../$KERNEL_OUT/arch/arm/boot/dts/amlogic/mesong9tv_n300.dtb \
        --output ./boot.img
    ls -l ./boot.img
    echo "boot.img done"
fi
