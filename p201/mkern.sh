...
#ROOTFS=$1
ROOTFS="out/target/product/p201/ramdisk.img"
...
KERNEL_OUT=out/target/product/p201/obj/KERNEL_OBJ
#mkdir -p $KERNEL_OUT
...
if [ "$2" != "m" ]; then
#    make -C common O=../$KERNEL_OUT gxbb_p201.dtd ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
    make -C common O=../$KERNEL_OUT gxbb_p201.dtb ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE
fi
...
if [ "$2" != "m" ]; then
    out/host/linux-x86/bin/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm64/boot/Image \
        --ramdisk ${ROOTFS} \
        --second common/../$KERNEL_OUT/arch/arm64/boot/dts/amlogic/gxbb_p201.dtb \
        --output ./out/target/product/p201/boot.img
    ls -l ./out/target/product/p201/boot.img
    echo "boot.img done"
fi
